...
## محدودسازی تلاش Login

برای جلوگیری از brute-force، به‌صورت پیش‌فرض حداکثر ۵ تلاش ناموفق برای هر
ترکیب Tenant و ایمیل در بازه ۶۰ ثانیه مجاز است. پس از آن API پاسخ `429` و هدر
`Retry-After` برمی‌گرداند. پس از Login موفق شمارنده همان کاربر پاک می‌شود.
